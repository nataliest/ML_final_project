...
def initialize_transformer(image_mean):
  ...
  transformer = caffe.io.Transformer({'data': shape}) # can create my own https://github.com/BVLC/caffe/blob/master/python/caffe/io.py
...
def singleFrame_classify_video(signal, net, transformer, with_smoothing, classNamesCNN):
    ...
    input_im = caffe.io.load_image(signal.replace(".wav",".png"))    # can create my own https://github.com/BVLC/caffe/blob/master/python/caffe/io.py    
